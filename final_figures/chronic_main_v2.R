...
figJ <- readRDS("../outs/rds/chronic_models.rds") + ggtitle("j")
...
figEF <- ((figE + subplotTheme + theme(plot.margin = margin(0, 0, 0, 0))) / 
  (figF + subplotTheme) + theme(plot.margin = margin(-20,0,0,0))) + 
  plot_layout(heights = c(2.75, 1)) + 
  plot_annotation(theme = theme(plot.margin = margin(30,0,0,0)))
...
p <- wrap_elements(figA + figB & subplotTheme) +
  ...
  wrap_elements(figJ + subplotTheme) +
