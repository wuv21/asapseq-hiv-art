...
createSeuListWithDsb <- function(samples, adtDir, filename, isoControls, sampleSuffix = "_count_out", seed = 21) {
  set.seed(seed)
  ...
  seuList <- lapply(samples, function(x) {
    ...
    adt_norm <- dsb::DSBNormalizeProtein(
      ...
      empty_drop_matrix = adt_matrix[, which(empty_calc$FDR >= 0.01)], 
      denoise.counts = TRUE, 
      use.isotype.control = TRUE, 
      isotype.control.name.vec = isoControls
    )
    
    adt <- CreateSeuratObject(counts = adt_filt, assay = "tsa", project = x)
    adt[["tsa"]]@data <- adt_norm
