...
t(adtART_matched@assays$tsa@data[c("A0154", "A0386", "A0390"), ]) %>% 
  as.data.frame(.) %>% 
  mutate(x = Cells(adtART_matched),
    y = adtART_matched$atacClusters) %>% 
  pivot_longer(cols = -one_of(c("x", "y")), names_to = "metric", values_to = "val") %>%
  group_by(metric, y) %>%
  summarize(mean = mean(val)) %>%
  arrange(metric, desc(y)) %>%
...
plotUmap(projART_matched, colorBy = "manualClusterAnnot", fn = "art_umap_labeledCluster_noPlotLabels", labelColors = FALSE, embedding = "UMAP2", propInLegend = TRUE)
...
iv_topPeaks_g <- ggplot(invitro_markerTest_activatedPeaks_topPeaks %>% slice_max(order_by = abs(piScore), n = 15),
...
getAssayData <- function(seu, cellNames, adtToUse) {
  ...
  sampleSplit <- caTools::sample.split(Y=df$haystackOut, SplitRatio = 0.7) 
...
calcRandomForest <- function(trainSet, sampleProps = list(c(NULL, NULL), c(3,1), c(2,1), c(1, 1), c(0.5, 1))) {
...
plotGgRoc(invitroModelsROC, "invitro_models")

data.frame(cleanName = tsa_catalog[tsa_catalog$DNA_ID %in% inVitroAdtToUse, "cleanName"],
  coeff = invitroLR$coefficients[-1],
  p = coef(summary(invitroLR))[-1, 4]) %>% 
  mutate(sig = case_when(
    p < 0.05 & coeff > 0 ~ "Up in HIV+",
    p < 0.05 & coeff < 0 ~ "Up in HIV-",
    TRUE ~ "ns"
  )) %>%
  ggplot(aes(x = coeff, y = -log10(p), color = sig)) +
  geom_point() +
  theme_classic() +
  scale_color_manual(values = c("#dddddd", HIVNEGCOLOR, HIVPOSCOLOR)) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "#cccccc") +
  ggrepel::geom_text_repel(data = . %>% filter(sig != "ns"),
    aes(label = cleanName), max.overlaps = 50, min.segment.length = 0.1, force = 2, size = 3) +
...
plotVolcanoFromGetMarkerFeatures(art_markerTest, chromVARmode = FALSE,
  fn = "art_volcano")
