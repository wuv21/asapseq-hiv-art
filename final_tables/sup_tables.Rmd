...
supTsaCatalog <- supTsaCatalog %>%
  select(DNA_ID, cleanName, Clone, Barcode, isCtrl) %>%
  rename(isControl = isCtrl,
...
**Supplemental Table 2:** Significant surface markers enriched in each cluster (over all other cells) for *in vitro* model (Level 1).
```{r, echo=FALSE, message=FALSE}
...
**Supplemental Table 3:** Significant surface markers enriched in HIV+ versus HIV- cells for *in vitro* model (Level 1).
```{r, echo=FALSE, message=FALSE}
...
\pagebreak
\blandscape
**Supplemental Table 4:** Differential peak list for activated CD4+ cells between HIV+ and HIV- for *in vitro* model (Level 1).
```{r, echo=FALSE, message=FALSE}
...
return(kbl(ivPeakListActivated, booktabs = T, align = "c", longtable = TRUE, linesep = "", escape = F) %>%
    kable_styling(latex_options = c("repeat_header")))
```
\elandscape
...
**Supplemental Table 5:** Significant surface markers enriched in each cluster (over all other cells) during chronic infection (Level 2).
```{r, echo=FALSE, message=FALSE}
...
**Supplemental Table 6:** Significant surface markers enriched in HIV+ versus HIV- cells during chronic infection (Level 2).
```{r, echo=FALSE, message=FALSE}
prepareDEMarkerTableCompiled(
  fns = c(
...
**Supplemental Table 7:** Significant surface markers enriched in each cluster (over all other cells) during treated infection (Level 3).
```{r, echo=FALSE, message=FALSE}
prepareDEMarkerTable("../outs/tsv/art_initClusterMarkers.tsv")
```
\pagebreak

**Supplemental Table 8:** Significant surface markers enriched in HIV+ versus HIV- cells during treated infection (Level 3).
```{r, echo=FALSE, message=FALSE}
prepareDEMarkerTableCompiled(
  fns = c(
