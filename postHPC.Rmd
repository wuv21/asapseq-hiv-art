...
getBaseATACPanel(projInvitro_matched, "outs/png/invitro_imputePanel.png", embedding = "UMAP2")
...
getBaseATACPanel(projChronic_matched, "outs/png/chronic_imputePanel.png", embedding = "UMAP2")
...
getBaseATACPanel(projART_matched, "outs/png/art_imputePanel.png", embedding = "UMAP2")
...
invitro_markerTest_activatedPeaks <- hiAnnotator::getNearestFeature(invitro_markerTest_activatedPeaks$Activated_TRUE, getGeneAnnotation(projInvitro_matched_gcPeak)[[1]], colnam = "symbol", feature.colnam = "symbol")
...
# qc metrics
```{r}
tmp <- data.frame(yaxis = getCellColData(projInvitro_matched, "log10(nFrags)")[,1],
  xaxis = getCellColData(projInvitro_matched, "as.character(haystackOut)")[,1]) %>%
  mutate(xaxis = factor(xaxis))


data.frame(yaxis = getCellColData(projInvitro_matched, "log10(nFrags)")[,1],
  xaxis = getCellColData(projInvitro_matched, "as.character(haystackOut)")[,1]) %>%
  ggplot(., aes(x = xaxis, y = yaxis, color = xaxis)) +
  geom_violin(aes(fill = xaxis), alpha = 0.4) +
  geom_boxplot(fill = "#FFFFFF00") +
  theme_classic() +
  stat_compare_means(method = "t.test", label.x.npc = 0.5, hjust = 0.5)


data.frame(yaxis = getCellColData(projInvitro_matched, "log10(nFrags)")[names(invitro_earlyCells),1],
  xaxis = getCellColData(projInvitro_matched, "as.character(haystackOut)")[names(invitro_earlyCells),1]) %>%
  ggplot(., aes(x = xaxis, y = yaxis, color = xaxis)) +
  geom_violin(aes(fill = xaxis), alpha = 0.4) +
  geom_boxplot(fill = "#FFFFFF00") +
  theme_classic() +
...
data.frame(yaxis = getCellColData(projInvitro_matched, "log10(nFrags)")[,1],
  xaxis = getCellColData(projInvitro_matched, "manualClusterAnnot")[,1]) %>%
  ggplot(., aes(x = xaxis, y = yaxis, color = xaxis)) +
  geom_violin(aes(fill = xaxis), alpha = 0.4) +
  geom_boxplot(fill = "#FFFFFF00") +
  theme_classic() +
...
data.frame(yaxis = getCellColData(projInvitro_matched, "TSSEnrichment")[,1],
  xaxis = getCellColData(projInvitro_matched, "as.character(haystackOut)")[,1]) %>%
  ggplot(., aes(x = xaxis, y = yaxis, color = xaxis)) +
  geom_violin(aes(fill = xaxis), alpha = 0.4) +
  geom_boxplot(fill = "#FFFFFF00") +
...
haystackChronic$viralFrags[haystackChronic$viralFrags$newCbc %in% projChronic_matched$cellNames, ] %>%
  mutate(individual = ifelse(grepl("C01", sample), "C01", "C02")) %>%
  group_by(newCbc) %>%
  summarize(n = n()) %>%
  mutate(yaxis = getCellColData(projChronic_matched, "TSSEnrichment")[newCbc, ]) %>%
  ggplot(aes(x = n, y = yaxis)) +
  geom_point()
  

data.frame(yaxis = getCellColData(projChronic_matched, "TSSEnrichment")[,1],
  xaxis = getCellColData(projChronic_matched, "as.character(haystackOut)")[,1]) %>%
  ggplot(., aes(x = xaxis, y = yaxis, color = xaxis)) +
  geom_violin(aes(fill = xaxis), alpha = 0.4) +
  geom_boxplot(fill = "#FFFFFF00") +
  theme_classic() +
  stat_compare_means(method = "wilcox", label.x.npc = 0.5, hjust = 0.5)
```
...
inVitroFrags_matched <- haystackInVitro$viralFrags[haystackInVitro$viralFrags$newCbc %in% projInvitro_matched$cellNames, ]
...
chronicFrags_matched <- haystackChronic$viralFrags[haystackChronic$viralFrags$newCbc %in% projChronic_matched$cellNames, ] %>%
  mutate(individual = ifelse(grepl("C01", sample), "C01", "C02"))
...
artFrags_matched <- haystackART$viralFrags[haystackART$viralFrags$newCbc %in% projART_matched$cellNames, ]
